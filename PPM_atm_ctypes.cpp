...
int myperspectiveProjection(double a, double b, GVector& sunpos_ecef, GVector& satpos_ecef, double& r_solar, double& area_bright, double& dis_boundary, double& dis_circle)
   {
       int state = -1;
        ...
        std::complex<double>* solutions = solve_quartic(ce[0], ce[1], ce[2], ce[3]);

        for(int i = 0 ; i< 4; i++)
        {
            if( fabs(solutions[i].imag()) == 0.0 )
            {
                XX[num_of_solution] = solutions[i].real();
                num_of_solution++;
            }
        }
...
// the main entry of the shadow function model
   double myshadowFactor(GVector& sunpos_eci, GVector& satpos_eci)
  {
           ...
           // for sphere test
           //double a = 6370.0; //km
           //double b = 6370.0; //km  6356.7523142
           //double a = 6371.0; //km
           //double b = 6371.0; //km  6356.7523142

           //double hgt_atm = 50.0; // the hight of atmosphere, this parameter is very important
           double hgt_atm = 13.0; // the hight of atmosphere, this parameter is very important
           //double hgt_atm = 0.0; // the hight of atmosphere, this parameter is very important
...
extern "C"
double shadow_function(double xsat, double ysat, double zsat, double xsun, double ysun, double zsun)
{
  // 2021/04/14 22:41  hgt=13
  // 22:41:11
  GVector satpos(xsat, ysat, zsat); // sat position in km
  GVector sunpos(xsun, ysun, zsun); //sun position in km
